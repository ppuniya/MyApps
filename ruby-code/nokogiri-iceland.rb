...
url = "http://groceries.iceland.co.uk/search?text=#{CGI.escape(searchstr)}&view=list"

doc = Nokogiri::HTML(open(url))
puts doc.at_css("title").text

minprice = doc.at_css(".tinyTxt")
if minprice
...
doc.css(".prod_list_item").each do |item|
	title = item.at_css('h4')
	if title
		title = title.text
	end
	price = item.at_css(".tinyTxt")
	if price
